...
const getThemeBorderClass = (theme: string) => {
  if (theme === 'hoangyen') return "border-[#cde8c6]/50";
...
const getThemeBgBorderTextClass = (theme: string) => {
  if (theme === 'hoangyen') return "bg-[#eff6f0]/90 border-[#cde8c6] text-[#1b3a1e] shadow-[1px_1px_0_0_#cde8c6]";
...
const getThemeAvatarFallbackIconClass = (theme: string) => {
  if (theme === 'hoangyen') return "w-4 h-4 text-[#ddd289]";
...
const getThemeAvatarBorderClass = (theme: string) => {
  if (theme === 'hoangyen') return "border border-[#cde8c6]";
...
const getThemeAvatarBgClass = (theme: string) => {
  if (theme === 'hoangyen') return "bg-white";
...
const getThemeAccentTextColor = (theme: string) => {
  if (theme === 'hoangyen') return "#ddd289";
...
const getThemeTitleBadgeClass = (theme: string) => {
  if (theme === 'hoangyen') return "bg-[#f4d451]/30 text-[#1b3a1e] border-[#f4d451]/50";
...
const getThemeSubTextColor = (theme: string) => {
  if (theme === 'hoangyen') return "text-[#1b3a1e]/60";
...
const getThemeGiftBadgeClass = (theme: string) => {
  if (theme === 'hoangyen') return "text-[#1b3a1e] bg-[#f4d451] border-[#f4d451]/50";
...
const getThemeContentTextColor = (theme: string) => {
  if (theme === 'hoangyen') return "text-[#1b3a1e]";
...
const getThemeReplyButtonClass = (theme: string) => {
  if (theme === 'hoangyen') return "text-[#1b3a1e]/80 hover:text-[#f4d451]";
...
const getThemeInputClass = (theme: string) => {
  if (theme === 'hoangyen') return "bg-white border-[#cde8c6] text-[#1b3a1e] focus:border-[#1b3a1e]";
...
const getThemeSendButtonClass = (theme: string) => {
  if (theme === 'hoangyen') return "bg-[#f4d451] hover:bg-[#ddd289] text-[#1b3a1e] border-[#1b3a1e] disabled:opacity-30";
...
export function Reader() {
  ...
  if (loading || !story || !currentChapter) {
    if (activeCustomTheme === 'hoangyen') {
      return (
        <div className="min-h-screen bg-[#eff6f0] text-[#132e1a] flex flex-col items-center justify-center p-4 font-serif">
    ...
    } else if (activeCustomTheme === 'homer') {
      return (
      ...
      return (
        <div className="min-h-screen bg-[#1b1715] text-[#d7cac1] flex flex-col items-center justify-center p-4 font-serif relative overflow-hidden">
          <div className="flex flex-col items-center gap-4 relative z-10">
            <div className="w-12 h-12 border-4 border-[#473a36] border-t-[#741611] rounded-full animate-spin" />
            <div className="text-xs uppercase tracking-[0.2em] text-[#741611] animate-pulse font-extrabold">
  ...
  const btnThemeClass = isCustomThemeActive ? (
    activeCustomTheme === 'hoangyen' ? "border-[#cde8c6] bg-[#eff6f0] hover:bg-[#aeed9a]/20 hover:border-[#1b3a1e] text-[#1b3a1e] shadow-[2px_2px_0_0_#cde8c6]" :
    ...
    activeCustomTheme === 'thaitu' ? "border-[#473a36]/50 bg-[#221c19] hover:bg-[#741611]/20 hover:border-[#741611] text-[#d7cac1] shadow-[2px_2px_0_0_#741611]" :
  ...
  const effectiveFontFamily = isCustomThemeActive ? (
    activeCustomTheme === 'hoangyen' ? 'font-reading-quicksand' : 
    ...
    activeCustomTheme === 'nguoidep' ? 'font-reading-lora' : 
    activeCustomTheme === 'thaitu' ? 'font-reading-cormorant' : 'font-reading-garamond'
  ) : fontFamily;
  const tChapterLabel = activeCustomTheme === 'hoangyen' ? "Khúc" : activeCustomTheme === 'homer' ? "Tọa độ" : activeCustomTheme === 'giagoan' ? "Tài liệu" : activeCustomTheme === 'nhatky' ? "Trang" : activeCustomTheme === 'thuytien' ? "Khúc tự luyến" : activeCustomTheme === 'rinhrap' ? "Màn" : activeCustomTheme === 'thientai' ? "Phó bản" : activeCustomTheme === 'nguoidep' ? "Bài luyện" : activeCustomTheme === 'thaitu' ? "Mật quyển" : "Chương";
  const tChapterCapLabel = activeCustomTheme === 'hoangyen' ? "KHÚC CA" : activeCustomTheme === 'homer' ? "TỌA ĐỘ" : activeCustomTheme === 'giagoan' ? "TÀI LIỆU" : activeCustomTheme === 'nhatky' ? "TRANG NHẬT KÝ" : activeCustomTheme === 'thuytien' ? "KHÚC TỰ LUYẾN" : activeCustomTheme === 'rinhrap' ? "MÀN CHƠI" : activeCustomTheme === 'thientai' ? "PHÓ BẢN" : activeCustomTheme === 'nguoidep' ? "BÀI ÔN LUYỆN" : activeCustomTheme === 'thaitu' ? "MẬT QUYỂN CHƯƠNG" : "CHƯƠNG";
  const tPrevChapter = activeCustomTheme === 'hoangyen' ? "Khúc nhạc trước" : activeCustomTheme === 'homer' ? "Tọa độ trước" : activeCustomTheme === 'giagoan' ? "Tài liệu trước" : activeCustomTheme === 'nhatky' ? "Trang nhật ký trước" : activeCustomTheme === 'thuytien' ? "Khúc tự luyến trước" : activeCustomTheme === 'rinhrap' ? "Màn trước" : activeCustomTheme === 'thientai' ? "Phó bản trước" : activeCustomTheme === 'nguoidep' ? "Bài ôn luyện trước" : activeCustomTheme === 'thaitu' ? "Quyển trước" : "Chương trước";
  const tNextChapter = activeCustomTheme === 'hoangyen' ? "Khúc nhạc sau" : activeCustomTheme === 'homer' ? "Tọa độ sau" : activeCustomTheme === 'giagoan' ? "Tài liệu sau" : activeCustomTheme === 'nhatky' ? "Trang nhật ký sau" : activeCustomTheme === 'thuytien' ? "Khúc tự luyến sau" : activeCustomTheme === 'rinhrap' ? "Màn sau" : activeCustomTheme === 'thientai' ? "Phó bản sau" : activeCustomTheme === 'nguoidep' ? "Bài ôn luyện sau" : activeCustomTheme === 'thaitu' ? "Quyển sau" : "Chương sau";
  const tChapterEarly = activeCustomTheme === 'hoangyen' ? "Khúc nhạc Đọc Sớm" : activeCustomTheme === 'homer' ? "Tọa độ Đọc Sớm" : activeCustomTheme === 'giagoan' ? "Tài liệu Đọc Sớm" : activeCustomTheme === 'nhatky' ? "Trang Đọc Sớm" : activeCustomTheme === 'thuytien' ? "Tự luyến Đọc Sớm" : activeCustomTheme === 'rinhrap' ? "Sinh tồn sớm" : activeCustomTheme === 'thientai' ? "Phó bản Đọc Sớm" : activeCustomTheme === 'nguoidep' ? "Bài ôn VIP" : activeCustomTheme === 'thaitu' ? "Sách văn Đọc Sớm" : "Chương Đọc Sớm";
  const tChapterLocked = activeCustomTheme === 'hoangyen' ? "Lồng chim đã khóa" : activeCustomTheme === 'homer' ? "Đã khóa tọa độ" : activeCustomTheme === 'giagoan' ? "Đã khóa tài liệu" : activeCustomTheme === 'nhatky' ? "Ký ức bị khép lại" : activeCustomTheme === 'thuytien' ? "Bóng gương chưa hiển hiện" : activeCustomTheme === 'rinhrap' ? "Màn bị ẩn" : activeCustomTheme === 'thientai' ? "Đã khóa phó bản" : activeCustomTheme === 'nguoidep' ? "Đã khóa bài ôn" : activeCustomTheme === 'thaitu' ? "Chưa triều duyệt công bố" : "Đã khóa chương";
  const tReadThis = activeCustomTheme === 'hoangyen' ? "Nghe khúc nhạc này" : activeCustomTheme === 'homer' ? "Đọc tọa độ này" : activeCustomTheme === 'giagoan' ? "Đọc tài liệu này" : activeCustomTheme === 'nhatky' ? "Lật giở ký ức này" : activeCustomTheme === 'thuytien' ? "Chiêm ngưỡng khúc ca này" : activeCustomTheme === 'rinhrap' ? "Chơi màn này" : activeCustomTheme === 'thientai' ? "Chinh phục phó bản này" : activeCustomTheme === 'nguoidep' ? "Luyện bài này" : activeCustomTheme === 'thaitu' ? "Duyệt lãm quyển này" : "Đọc chương này";
  const tChapterSingle = activeCustomTheme === 'hoangyen' ? "khúc nhạc" : activeCustomTheme === 'homer' ? "tọa độ" : activeCustomTheme === 'giagoan' ? "tài liệu" : activeCustomTheme === 'nhatky' ? "trang" : activeCustomTheme === 'thuytien' ? "khúc" : activeCustomTheme === 'rinhrap' ? "màn" : activeCustomTheme === 'thientai' ? "phó bản" : activeCustomTheme === 'nguoidep' ? "đề bài" : activeCustomTheme === 'thaitu' ? "quyển" : "chương";
  const tParagraphComment = activeCustomTheme === 'hoangyen' ? "Phản hồi chim hót" : activeCustomTheme === 'homer' ? "Phản hồi tọa độ" : activeCustomTheme === 'giagoan' ? "Ý kiến đoạn" : activeCustomTheme === 'nhatky' ? "Bút tích đoạn" : activeCustomTheme === 'thuytien' ? "Lời soi bóng" : activeCustomTheme === 'rinhrap' ? "Tiếng động đoạn" : activeCustomTheme === 'thientai' ? "Phát kiến phó bản" : activeCustomTheme === 'nguoidep' ? "Nhận xét đáp án" : activeCustomTheme === 'thaitu' ? "Mật tấu đoạn" : "Bình luận đoạn";
  const tNoParagraphComment = activeCustomTheme === 'hoangyen' ? "Đoạn này chưa có chim hoàng yến hót..." : activeCustomTheme === 'homer' ? "Chưa có phản hồi nào cho tọa độ này." : activeCustomTheme === 'giagoan' ? "Chưa có ý kiến nào cho đoạn này." : activeCustomTheme === 'nhatky' ? "Chưa có dấu bút tích nào tại dòng ký ức này." : activeCustomTheme === 'thuytien' ? "Chưa có lời soi bóng nào được lưu lại bên đoạn tuyệt mỹ này." : activeCustomTheme === 'rinhrap' ? "Đoạn này hoàn toàn im lặng..." : activeCustomTheme === 'thientai' ? "Chưa có phát kiến nào cho tọa độ này." : activeCustomTheme === 'nguoidep' ? "Chưa có nhận xét nào cho bài giải này." : activeCustomTheme === 'thaitu' ? "Khuyết khuyết tấu biểu đoạn văn này..." : "Chưa có bình luận. Hãy là người đầu tiên!";
  const tParagraphPlaceholder = activeCustomTheme === 'hoangyen' ? "Viết phản hồi hót líu lo..." : activeCustomTheme === 'homer' ? "Viết phản hồi..." : activeCustomTheme === 'giagoan' ? "Viết đề xuất..." : activeCustomTheme === 'nhatky' ? "Để lại dòng bút tích..." : activeCustomTheme === 'thuytien' ? "Để lại lời ca tụng nhan sắc..." : activeCustomTheme === 'rinhrap' ? "Tạo tiếng động..." : activeCustomTheme === 'thientai' ? "Ghi lại phát kiến..." : activeCustomTheme === 'nguoidep' ? "Ghi chú bài giải..." : activeCustomTheme === 'thaitu' ? "Dâng mật nghị đoạn..." : "Viết bình luận...";
  const tParagraphSubmit = activeCustomTheme === 'hoangyen' ? "Hót" : activeCustomTheme === 'homer' ? "Phát" : activeCustomTheme === 'giagoan' ? "Trình" : activeCustomTheme === 'nhatky' ? "Ghi" : activeCustomTheme === 'thuytien' ? "Soi" : activeCustomTheme === 'rinhrap' ? "Phát" : activeCustomTheme === 'nguoidep' ? "Nộp" : activeCustomTheme === 'thaitu' ? "Tấu" : "Gửi";
  const tParagraphLogin = activeCustomTheme === 'hoangyen' ? "Đăng nhập để hót cùng" : activeCustomTheme === 'homer' ? "Đăng nhập để phản hồi" : activeCustomTheme === 'giagoan' ? "Đăng nhập để trình ý kiến" : activeCustomTheme === 'nhatky' ? "Đăng nhập để lại bút tích" : activeCustomTheme === 'thuytien' ? "Đăng nhập để tự soi bóng" : activeCustomTheme === 'rinhrap' ? "Đăng nhập để tạo tiếng động" : activeCustomTheme === 'nguoidep' ? "Đăng nhập để nhận xét" : activeCustomTheme === 'thaitu' ? "Đăng nhập để dâng tấu trình" : "Đăng nhập để bình luận";
  const tCommentAreaTitle = activeCustomTheme === 'hoangyen' ? "Bản hòa tấu đàm luận" : activeCustomTheme === 'homer' ? "Báo cáo sóng phản hồi" : activeCustomTheme === 'giagoan' ? "Đề xuất và ý kiến" : activeCustomTheme === 'nhatky' ? "Dấu vết bút tích chương" : activeCustomTheme === 'thuytien' ? "Mặt hồ tự chiêm ngưỡng" : activeCustomTheme === 'rinhrap' ? "Dấu vết để lại" : activeCustomTheme === 'thientai' ? "Báo cáo chiến thuật phó bản" : activeCustomTheme === 'nguoidep' ? "Báo cáo tiến độ ôn tập" : activeCustomTheme === 'thaitu' ? "Tấu chương đàm luận" : "Bình luận chương";
  const tCommentAreaPlaceholder = activeCustomTheme === 'hoangyen' ? "Để lại tiếng hót líu lo đàm đạo cùng Lê Phi Phàm..." : activeCustomTheme === 'homer' ? "Nhập sóng phản hồi của bạn..." : activeCustomTheme === 'giagoan' ? "Nhập đề xuất và ý kiến của bạn..." : activeCustomTheme === 'nhatky' ? "Nhập bút tích thương nhớ của bạn..." : activeCustomTheme === 'thuytien' ? "Khắc ghi dòng chiêm ngưỡng bản thân bên bờ nước thẳm..." : activeCustomTheme === 'rinhrap' ? "Để lại dấu vết của bạn..." : activeCustomTheme === 'thientai' ? "Nhập chiến thuật của bạn..." : activeCustomTheme === 'nguoidep' ? "Ghi nhận xét của bạn..." : activeCustomTheme === 'thaitu' ? "Dâng tấu chương đóng góp ý kiến đàm luận tại chương này..." : "Nhập bình luận của bạn...";
  const tCommentAreaPlaceholderLogin = activeCustomTheme === 'hoangyen' ? "Đăng nhập để hót líu lo đàm đạo..." : activeCustomTheme === 'homer' ? "Đăng nhập để phát sóng phản hồi" : activeCustomTheme === 'giagoan' ? "Đăng nhập để trình ý kiến" : activeCustomTheme === 'nhatky' ? "Đăng nhập để ghi lại dòng hoài niệm" : activeCustomTheme === 'thuytien' ? "Đăng nhập để ca tụng dung nhan chính mình" : activeCustomTheme === 'rinhrap' ? "Đăng nhập để để lại dấu vết" : activeCustomTheme === 'thientai' ? "Đăng nhập để chia sẻ chiến thuật" : activeCustomTheme === 'nguoidep' ? "Đăng nhập để nộp ý kiến" : activeCustomTheme === 'thaitu' ? "Đăng nhập để dâng tấu chương đàm luận..." : "Đăng nhập để bình luận";
       ...
       {isCustomThemeActive && (
         <>
           <div className="fixed inset-0 pointer-events-none opacity-20"
                style={{ backgroundImage: activeCustomTheme === 'hoangyen'
                   ? 'repeating-linear-gradient(45deg, #cde8c6 0, #cde8c6 1px, transparent 1px, transparent 60px)'
                  ...
                  : activeCustomTheme === 'thaitu'
                  ? 'repeating-linear-gradient(45deg, #473a36 0, #473a36 1px, transparent 1px, transparent 60px)'
           ...
           <div className="fixed top-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full blur-[150px] opacity-30 pointer-events-none"
                style={activeCustomTheme === 'hoangyen' ? { backgroundColor: '#aeed9a' } : activeCustomTheme === 'homer' ? { backgroundColor: '#47515f' } : activeCustomTheme === 'nhatky' ? { backgroundColor: '#D5C2A5' } : activeCustomTheme === 'thuytien' ? { backgroundColor: '#2E251E' } : activeCustomTheme === 'rinhrap' ? { backgroundColor: rinhrapMode === 'thotrang' ? '#facaca' : '#450A0A' } : activeCustomTheme === 'thientai' ? { backgroundColor: '#9a858d' } : activeCustomTheme === 'nguoidep' ? { backgroundColor: '#2D3D54' } : activeCustomTheme === 'thaitu' ? { backgroundColor: '#741611' } : { backgroundColor: '#2e2a63' }} />
           <div className="fixed bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full blur-[180px] opacity-10 pointer-events-none"
                style={activeCustomTheme === 'hoangyen' ? { backgroundColor: '#f4d451' } : activeCustomTheme === 'homer' ? { backgroundColor: '#67707e' } : activeCustomTheme === 'nhatky' ? { backgroundColor: '#BCA782' } : activeCustomTheme === 'thuytien' ? { backgroundColor: '#9A8E7D' } : activeCustomTheme === 'rinhrap' ? { backgroundColor: rinhrapMode === 'thotrang' ? '#780606' : '#991B1B' } : activeCustomTheme === 'thientai' ? { backgroundColor: '#645a6c' } : activeCustomTheme === 'nguoidep' ? { backgroundColor: '#151C28' } : activeCustomTheme === 'thaitu' ? { backgroundColor: '#473a36' } : { backgroundColor: '#bbee1f' }} />
         </>
       )}
       {/* Top Navigation */}
          ...
          <div className="relative flex items-center gap-2">
             ...
             {showSettings && (
                <div className={cn("absolute right-0 top-full mt-2 w-64 p-5 rounded-3xl shadow-xl z-20 border-3 transition-colors",
                    isCustomThemeActive ? (
                       activeCustomTheme === 'hoangyen' ? "bg-[#eff6f0] text-[#1b3a1e] border-[#cde8c6] shadow-lg border-2 font-reading-quicksand" : activeCustomTheme === 'homer' ? "bg-[#181f2d] text-[#a0a6b3] border-[#47515f] font-reading-garamond shadow-lg" :
                       ...
                       activeCustomTheme === 'thientai' ? "bg-[#060406] text-[#d4c6c9] border-[#34282d] font-reading-iosevka shadow-lg" : activeCustomTheme === 'nguoidep' ? "bg-[#101622] text-[#ECEFF4] border-[#2D3D54]/40 font-reading-lora shadow-lg" : activeCustomTheme === 'thaitu' ? "bg-[#1b1715] text-[#d7cac1] border-[#473a36] font-serif shadow-2xl border-2" : "bg-[#13120d] text-[#dbcec2] border-[#2e2a63] font-reading-garamond shadow-lg"
                    ) : effectiveIsDark ? "bg-[#211B18] text-[#ECE5DC] border-[#3E2723]" : "bg-[#FFFDF9] text-[#3E2723] border-[#3E2723]"
                )}>
                   <h3 className={cn("font-black mb-4 uppercase tracking-wider text-sm", isCustomThemeActive && activeCustomTheme === 'nguoidep' && "font-alegreya font-black text-base text-[#c5ad97]", isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "text-[#1b3a1e]" : activeCustomTheme === 'homer' ? "text-[#a0a6b3]" : activeCustomTheme === 'nhatky' ? "text-[#2C1814]" : activeCustomTheme === 'thuytien' ? "text-[#B6A996]" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "text-[#780606]" : "text-[#991B1B]") : activeCustomTheme === 'thientai' ? "text-[#9a858d]" : activeCustomTheme === 'nguoidep' ? "text-[#c5ad97]" : activeCustomTheme === 'thaitu' ? "text-[#741611]" : "text-[#bbee1f]") : (effectiveIsDark ? "text-[#ECE5DC]" : "text-[#3E2723]"))}>Cài đặt</h3>
                   <div className="flex flex-col gap-6">
                      {hasCustomTheme && (
                         <div className={cn("flex flex-col gap-2 pb-4 border-b-2 border-dashed", activeCustomTheme === 'hoangyen' ? 'border-[#cde8c6]/30' : activeCustomTheme === 'homer' ? 'border-[#47515f]/20' : activeCustomTheme === 'nhatky' ? 'border-[#BCA782]/30' : activeCustomTheme === 'thuytien' ? 'border-[#B6A996]/20' : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? 'border-[#facaca]/30' : 'border-[#7F1D1D]/30') : activeCustomTheme === 'thientai' ? 'border-[#34282d]/30' : activeCustomTheme === 'nguoidep' ? 'border-[#2D3D54]/30' : activeCustomTheme === 'thaitu' ? 'border-[#741611]/30' : 'border-[#2e2a63]/20')}>
                            <label className={cn("text-xs font-black uppercase flex items-center justify-between", isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "text-[#1b3a1e]" : activeCustomTheme === 'homer' ? "text-[#a0a6b3]" : activeCustomTheme === 'nhatky' ? "text-[#2C1814]" : activeCustomTheme === 'thuytien' ? "text-[#B6A996]" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "text-[#780606]" : "text-[#991B1B]") : activeCustomTheme === 'thientai' ? "text-[#9a858d]" : activeCustomTheme === 'nguoidep' ? "text-[#ECEFF4]" : activeCustomTheme === 'thaitu' ? "text-[#741611]" : "text-[#bbee1f]") : (effectiveIsDark ? "text-[#D7CCC8]" : "text-[#8D6E63]"))}>
                               Giao diện truyện
                               {activeCustomTheme === 'hoangyen' ? <span className="text-[9px] bg-[#f4d451] text-[#1b3a1e] font-bold px-1.5 py-0.5 rounded border border-[#1b3a1e]">CHIM HÓT</span> : activeCustomTheme === 'homer' ? <span className="text-[9px] bg-[#a0a6b3] text-[#181f2d] font-bold px-1.5 py-0.5 rounded border border-[#a0a6b3]">PREMIUM</span> : activeCustomTheme === 'nhatky' ? <span className="text-[9px] bg-[#DFCEB4] text-[#2C1814] font-bold px-1.5 py-0.5 rounded border border-[#BCA782]">PREMIUM</span> : activeCustomTheme === 'thuytien' ? <span className="text-[9px] bg-[#B6A996] text-[#12110F] font-bold px-1.5 py-0.5 rounded border border-[#B6A996]">PREMIUM</span> : activeCustomTheme === 'rinhrap' ? <span className={cn("text-[9px] font-bold px-1.5 py-0.5 rounded border", rinhrapMode === 'thotrang' ? "bg-[#facaca] text-[#780606] border-[#facaca]" : "bg-[#7F1D1D] text-white border-[#7F1D1D]")}>PREMIUM</span> : activeCustomTheme === 'thientai' ? <span className="text-[9px] bg-[#9a858d] text-[#060406] font-bold px-1.5 py-0.5 rounded border border-[#9a858d]">VR GAME</span> : activeCustomTheme === 'nguoidep' ? <span className="text-[9px] bg-[#A2B6CD] text-[#101622] font-bold px-1.5 py-0.5 rounded border border-[#2D3D54]/50">SĨ TỬ</span> : activeCustomTheme === 'thaitu' ? <span className="text-[9px] bg-[#741611] text-[#d7cac1] font-bold px-1.5 py-0.5 rounded border border-[#741611]">HOÀNG TỘC</span> : <span className="text-[9px] bg-[#bbee1f] text-[#13120d] font-bold px-1.5 py-0.5 rounded border border-[#bbee1f]">PREMIUM</span>}
                            </label>
                            <button 
                               type="button"
                      ...
                      <div className="flex items-center justify-between">
                         <label className={cn("text-xs font-black uppercase", isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "text-[#1b3a1e]/60" : activeCustomTheme === 'homer' ? "text-[#67707e]" : activeCustomTheme === 'nhatky' ? "text-[#5C3627]/80" : activeCustomTheme === 'thuytien' ? "text-[#B6A996]/80" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "text-[#780606]/80" : "text-[#7F1D1D]/80") : activeCustomTheme === 'thientai' ? "text-[#9a858d]/80" : activeCustomTheme === 'nguoidep' ? "text-[#A2B6CD]" : activeCustomTheme === 'thaitu' ? "text-[#8c7b72]" : "text-[#695b7f]") : (effectiveIsDark ? "text-[#D7CCC8]" : "text-[#8D6E63]"))}>Chế độ nền tối</label>
                      ...
                      <div>
                         <label className={cn("text-xs font-black uppercase mb-3 block", isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "text-[#1b3a1e]/60" : activeCustomTheme === 'homer' ? "text-[#67707e]" : activeCustomTheme === 'nhatky' ? "text-[#5C3627]/80" : activeCustomTheme === 'thuytien' ? "text-[#B6A996]/80" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "text-[#780606]/80" : "text-[#7F1D1D]/80") : activeCustomTheme === 'thientai' ? "text-[#9a858d]/80" : activeCustomTheme === 'nguoidep' ? "text-[#A2B6CD]" : activeCustomTheme === 'thaitu' ? "text-[#8c7b72]" : "text-[#695b7f]") : (effectiveIsDark ? "text-[#D7CCC8]" : "text-[#8D6E63]"))}>Cỡ chữ: {fontSize}px</label>
<input type="range" min="14" max="28" value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))} className={cn("w-full accent-[#3E2723]", isCustomThemeActive && (activeCustomTheme === 'hoangyen' ? "accent-[#f4d451]" : activeCustomTheme === 'homer' ? "accent-[#a0a6b3]" : activeCustomTheme === 'nhatky' ? "accent-[#2C1814]" : activeCustomTheme === 'thuytien' ? "accent-[#B6A996]" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "accent-[#780606]" : "accent-[#991B1B]") : activeCustomTheme === 'thientai' ? "accent-[#9a858d]" : activeCustomTheme === 'nguoidep' ? "accent-[#A2B6CD]" : activeCustomTheme === 'thaitu' ? "accent-[#741611]" : "accent-[#bbee1f]"))} />
                      </div>
                      <div>
                         <label className={cn("text-xs font-black uppercase mb-3 block", isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "text-[#1b3a1e]/60" : activeCustomTheme === 'homer' ? "text-[#67707e]" : activeCustomTheme === 'nhatky' ? "text-[#5C3627]/80" : activeCustomTheme === 'thuytien' ? "text-[#B6A996]/80" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "text-[#780606]/80" : "text-[#7F1D1D]/80") : activeCustomTheme === 'thientai' ? "text-[#9a858d]/80" : activeCustomTheme === 'nguoidep' ? "text-[#A2B6CD]" : activeCustomTheme === 'thaitu' ? "text-[#8c7b72]" : "text-[#695b7f]") : (effectiveIsDark ? "text-[#D7CCC8]" : "text-[#8D6E63]"))}>Phông chữ</label>
                         <select 
                           value={fontFamily} 
                           ...
                           className={cn(
                             ...
                             isCustomThemeActive ? (
                               activeCustomTheme === 'hoangyen' ? "bg-[#eff6f0] text-[#1b3a1e] border-[#cde8c6] cursor-not-allowed opacity-75" : activeCustomTheme === 'homer' ? "bg-[#181f2d] text-[#a0a6b3] border-[#47515f] cursor-not-allowed opacity-75" :
                               ...
                               activeCustomTheme === 'nguoidep' ? "bg-[#101622] text-[#ECEFF4] border-[#2D3D54]/50 cursor-not-allowed opacity-75" : activeCustomTheme === 'thaitu' ? "bg-[#1b1715] text-[#d7cac1] border-[#473a36] cursor-not-allowed opacity-75" :
                         ...
                         >
                          ...
                          {isCustomThemeActive && <span className={cn("text-[10px] mt-1 block", activeCustomTheme === 'hoangyen' ? "text-[#1b3a1e]" : activeCustomTheme === 'homer' ? "text-[#a0a6b3]" : activeCustomTheme === 'nhatky' ? "text-[#2C1814]" : activeCustomTheme === 'thuytien' ? "text-[#EADDC9]" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "text-[#780606]" : "text-[#D8B4B4]") : activeCustomTheme === 'thientai' ? "text-[#9a858d]" : activeCustomTheme === 'nguoidep' ? "text-[#A2B6CD]" : activeCustomTheme === 'thaitu' ? "text-[#8c7b72]" : "text-[#bbee1f]")}>{activeCustomTheme === 'hoangyen' ? 'Khóa phông chữ Quicksand theo theme' : activeCustomTheme === 'homer' ? 'Khóa phông chữ Iosevka theo theme' : activeCustomTheme === 'nhatky' ? 'Khóa phông chữ Cormorant Garamond theo theme' : activeCustomTheme === 'thuytien' ? 'Khóa phông chữ Cormorant Garamond theo theme' : activeCustomTheme === 'rinhrap' ? 'Khóa phông chữ Quicksand theo theme' : activeCustomTheme === 'thientai' ? 'Khóa phông chữ Iosevka theo theme' : activeCustomTheme === 'nguoidep' ? 'Khóa phông chữ Lora & Alegreya theo theme' : activeCustomTheme === 'thaitu' ? 'Khóa phông chữ Noto Serif theo theme' : 'Khóa phông chữ EB Garamond theo theme'}</span>}
                      </div>
                   </div>
                </div>
             )}
       ...
       <main className="w-full max-w-2xl px-6 md:px-8 py-12 flex-1 relative z-0">
          <article 
              className={cn(
                  ...
                  isCustomThemeActive
                       ? (activeCustomTheme === 'hoangyen' ? "prose-p:text-[#1b3a1e] prose-h2:text-[#1b3a1e] prose-blockquote:border-[#cde8c6] prose-blockquote:text-[#1b3a1e]/80" : activeCustomTheme === 'homer'
                           ...
                           : activeCustomTheme === 'nhatky'
                               ...
                               : activeCustomTheme === 'thuytien'
                                   ...
                                   : activeCustomTheme === 'rinhrap'
                                       ...
                                       : activeCustomTheme === 'thientai' ? "prose-p:text-[#d4c6c9] prose-h2:text-[#9a858d] prose-blockquote:border-[#645a6c] prose-blockquote:text-[#9a858d]" : activeCustomTheme === 'nguoidep' ? "prose-p:text-[#ECEFF4]/90 prose-h2:text-[#A2B6CD] prose-blockquote:border-[#2D3D54]/40 prose-blockquote:text-[#ECEFF4]/70" : activeCustomTheme === 'thaitu' ? "prose-p:text-[#d7cac1] prose-h2:text-[#741611] prose-blockquote:border-[#741611]/60 prose-blockquote:text-[#8c7b72]" : "prose-p:text-[#dbcec2] prose-h2:text-[#bbee1f] prose-blockquote:border-[#2e2a63] prose-blockquote:text-[#695b7f]")
                        : effectiveIsDark 
                        ...
                        <div key={idx} className="relative group/para">
                           <p className="relative w-full cursor-pointer md:cursor-auto mb-6" onClick={() => {
                               ...
                               <button 
                                   ...
                                   className={cn("inline-flex ml-3 items-center justify-center opacity-40 md:opacity-0 md:group-hover/para:opacity-100 transition-opacity hover:opacity-100 align-baseline cursor-pointer", isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "hover:text-[#132e1a]" : activeCustomTheme === 'homer' ? "hover:text-[#a0a6b3]" : activeCustomTheme === 'nhatky' ? "hover:text-[#E8DCB8]" : activeCustomTheme === 'thuytien' ? "hover:text-[#B6A996]" : activeCustomTheme === 'rinhrap' ? "hover:text-[#EF4444]" : activeCustomTheme === 'thientai' ? "hover:text-[#9a858d]" : activeCustomTheme === 'nguoidep' ? "hover:text-[#A2B6CD]" : activeCustomTheme === 'thaitu' ? "hover:text-[#741611]" : "hover:text-[#bbee1f]") : "hover:text-[#8D6E63]", pComments.length > 0 && (isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "opacity-100 md:opacity-100 text-[#132e1a]" : activeCustomTheme === 'homer' ? "opacity-100 md:opacity-100 text-[#a0a6b3]" : activeCustomTheme === 'nhatky' ? "opacity-100 md:opacity-100 text-[#E8DCB8]" : activeCustomTheme === 'thuytien' ? "opacity-100 md:opacity-100 text-[#B6A996]" : activeCustomTheme === 'rinhrap' ? "opacity-100 md:opacity-100 text-[#EF4444]" : activeCustomTheme === 'thientai' ? "opacity-100 md:opacity-100 text-[#9a858d]" : activeCustomTheme === 'nguoidep' ? "opacity-100 md:opacity-100 text-[#A2B6CD]" : activeCustomTheme === 'thaitu' ? "opacity-100 md:opacity-100 text-[#741611]" : "opacity-100 md:opacity-100 text-[#bbee1f]") : "opacity-100 md:opacity-100 text-[#8D6E63]"))}
                               >
                           ...
                           {activeParagraphIndex === idx && (
                               <div className={cn("mt-4 p-5 rounded-2xl border-2 transition-all", 
                                    isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "bg-[#eff6f0]/95 border-[#cde8c6] text-[#1b3a1e] shadow-[2px_2px_0_0_#cde8c6]" : activeCustomTheme === 'homer' ? "bg-[#181f2d]/95 border-[#47515f] text-[#a0a6b3] shadow-[2px_2px_0_0_#47515f]" : activeCustomTheme === 'nhatky' ? "bg-[#E8DCC4] border-[#BCA782] text-[#2C1814] shadow-[2px_2px_0_0_#BCA782]" : activeCustomTheme === 'thuytien' ? "bg-[#12110F]/95 border-[#B6A996] text-[#F2E6D0] shadow-[2px_2px_0_0_#12110F]" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "bg-[#fff2f1]/95 border-[#facaca] text-[#780606] shadow-[2px_2px_0_0_#facaca]" : "bg-[#100707]/95 border-[#7F1D1D] text-[#D8B4B4] shadow-[2px_2px_0_0_#450A0A]") : activeCustomTheme === 'thientai' ? "bg-[#060406]/95 border-[#9a858d] text-[#d4c6c9] shadow-[2px_2px_0_0_#9a858d]" : activeCustomTheme === 'nguoidep' ? "bg-[#101622]/95 border-[#2D3D54]/40 text-[#ECEFF4] shadow-[2px_2px_0_0_#2D3D54]" : activeCustomTheme === 'thaitu' ? "bg-[#14100e]/95 border-[#741611]/60 text-[#d7cac1] shadow-[2px_2px_0_0_#741611]" : "bg-[#13120d]/95 border-[#2e2a63] text-[#dbcec2] shadow-[2px_2px_0_0_#2e2a63]") 
                                       : effectiveIsDark 
                                          ? "bg-[#120E0C] border-[#3E2723] shadow-[2px_2px_0_0_#3E2723]" 
                                          : "bg-[#FFFDF9] border-[#3E2723] shadow-[2px_2px_0_0_#3E2723]"
                                )}>
                                   <div className="flex justify-between items-center mb-3">
                                       <h4 className={cn("text-sm font-bold uppercase tracking-wider", isCustomThemeActive ? (activeCustomTheme === 'hoangyen' ? "text-[#1b3a1e]" : activeCustomTheme === 'homer' ? "text-[#a0a6b3]" : activeCustomTheme === 'nhatky' ? "text-[#2C1814]" : activeCustomTheme === 'thuytien' ? "text-[#B6A996]" : activeCustomTheme === 'rinhrap' ? (rinhrapMode === 'thotrang' ? "text-[#780606]" : "text-[#EF4444]") : activeCustomTheme === 'thientai' ? "text-[#9a858d]" : activeCustomTheme === 'nguoidep' ? "text-[#A2B6CD]" : activeCustomTheme === 'thaitu' ? "text-[#741611]" : "text-[#bbee1f]") : (effectiveIsDark ? "text-[#D7CCC8]" : "text-[#8D6E63]"))}>{tParagraphComment}</h4>
