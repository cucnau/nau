...
export function Store() {
  const { choco, goldenChoco, spendChoco, spendGoldenChoco, addGoldenChoco, isLoggedIn, email, buyTicket, updateUserDoc, ownedStickers, addOwnedSticker, equipSticker, firebaseUser, ownedAccessories, addOwnedAccessory, equipAccessory } = useStore();
  const [storeStickers, setStoreStickers] = useState<any[]>([]);
  const [storeAccessories, setStoreAccessories] = useState<any[]>([]);
  const [activeTab, setActiveTab] = useState<'items' | 'stickers' | 'accessories'>('items');
  ...
  useEffect(() => {
    ...
    const fetchAccessories = async () => {
      try {
        const q = query(collection(db, 'store_accessories'), orderBy('createdAt', 'desc'));
        const snap = await getDocs(q);
        setStoreAccessories(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
      } catch (err) {
  ...
  const filteredItems = activeTab === 'stickers'
     ? storeStickers.filter(sticker => (sticker.name || '').toLowerCase().includes(searchQuery.toLowerCase()))
     : storeAccessories.filter(acc => (acc.name || '').toLowerCase().includes(searchQuery.toLowerCase()));
                ...
                {displayedItems.map((accessory) => (
                   <div key={accessory.id} className="bg-white border border-[#D7CCC8] rounded-2xl p-4 sm:p-5 flex flex-col items-center text-center shadow-sm relative overflow-hidden group hover:border-[#8D6E63] transition-colors">
                      ...
                      {ownedAccessories?.includes(accessory.url) ? (
                         ...
                         <button onClick={() => buyAccessory(accessory)} className={cn("p-2 sm:px-4 sm:py-2.5 rounded-xl font-bold transition-colors w-full mt-auto flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 uppercase text-[10px] sm:text-xs tracking-widest shadow-md", accessory.type === 'golden' ? "bg-[#D4AF37] text-white hover:bg-[#B5952F]" : "bg-[#3E2723] text-[#FDF6EC] hover:bg-[#2D1B19]")}>
                            <span>Mua</span> <span className={cn("px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] whitespace-nowrap", accessory.type === 'golden' ? "bg-black/10" : "bg-[#FDF6EC]/20")}>{accessory.price} {accessory.type === 'golden' ? 'GChoco' : 'Choco'}</span>
                         </button>
                      )}
                   </div>
                ))}
